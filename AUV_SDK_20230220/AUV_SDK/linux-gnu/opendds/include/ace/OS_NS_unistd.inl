...
ACE_OS::chdir (const char *path)
{
  ...
  return MQX_Filesystem::inst ().chdir (path);
#else
...
ACE_OS::rmdir (const char *path)
{
  ...
  ACE_WIN32CALL_RETURN (ACE_ADAPT_RETVAL (::RemoveDirectory (ACE_TEXT_CHAR_TO_TCHAR(path)),
                        ...
                        int, -1);
#elif defined (ACE_MQX)
  return MQX_Filesystem::inst ().rmdir (path);
#elif defined (ACE_LACKS_RMDIR)
  ACE_NOTSUP_RETURN (-1);
#else
...
#elif defined (ACE_MQX)
  return MQX_Filesystem::inst ().getcwd (buf, size);
#else
...
# elif defined (ACE_HAS_WINCE)
  ...
  ACE_WIN32CALL_RETURN (ACE_ADAPT_RETVAL (::DeleteFile (ACE_TEXT_CHAR_TO_TCHAR (path)), ace_result_),
                        int, -1);
# elif defined (ACE_MQX)
  return MQX_Filesystem::inst ().unlink (path);
